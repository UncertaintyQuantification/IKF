# this is a function is copy from the package
...
# predict the trajectory of Vicsek model
Vicsek_pred_trajec = function(est_param,v1,p1,v_abs,n_t,T_test,h,sigma_0,weights,theta_train_vec,kernel_type = 'matern_5_2',tilde_nu=0.1,noise_type='Gaussian'){
  beta=est_param[1]
  tau=est_param[2]
  ...
  for(t in 1:T_test){
    ...
    theta_here = atan2(v_vec_here[2,],v_vec_here[1,])
    
    ##augment it for fast prediction 
    d_aug=c(theta_here,(theta_train_vec))
    ...
    pred_mean_aug=Get_R_y_with_kernel_type(beta, tilde_nu, delta_x_aug, w_aug[d_aug_sort$ix], kernel_type=kernel_type)
    
    pred_mean_fast=pred_mean_aug[d_aug_sort_rev_ix][1:n_t]
    ...
    for(i in 1:n_t){
      ...
      theta_neighbor=pred_mean_fast[index_neighbor]
      ...
      #n_neighbor=c(n_neighbor,length(index_neighbor))
  ...
  return(list(pos_pred=pos,v_pred=v,theta_pred=theta))
}

# perform residual bootstrap in Vicsek model
residual_bootstrap_Vicsek = function(est, sim, B = 100, 
  ...
  theta_all_vec = est@training_data$training_angles
  A_all_vec = est@training_data$A_theta
  num_neighbors_all_vec = est@training_data$num_neighbors
  
  sort_theta_all=sort(theta_all_vec,index.return=T)
  N_tilde_all=length(theta_all_vec)
  delta_x_all=sort_theta_all$x[-1]-sort_theta_all$x[-N_tilde_all]
  
  d_sort_rev_ix=sort(sort_theta_all$ix,index.return=T)$ix ###this is to reverse the previous sort 
  
  output_all=unlist(sim@theta_list[1+1:T_sim])
  ...
  pred_mean_rev = Get_R_y_with_kernel_type(beta, tilde_nu, delta_x=delta_x_all, output=w_CG[sort_theta_all$ix], kernel_type=kernel_type)
  
  pred_mean_fast = pred_mean_rev[d_sort_rev_ix]
  
  pred_mean_output=A_times_x_particle( pred_mean_fast,  A_all_vec,  num_neighbors_all_vec,  
                                       D_y,  N)
  ...
  for(b in 1:B){
    ...
    est_new = fit(sim,param=param_ini,cut_r_max=cut_r_max, nx=nx, ny=ny,
                  kernel_type = kernel_type, tol = tol, 
                  output = output_new, ho_output = ho_output_new,testing_inputs=NULL)
...
residual_bootstrap_with_prediction_cell <- function(exp_data, est, B, n_record, output_all, sigma_2_record, 
  ...
  pred_mean_rev = Get_R_y_with_kernel_type(beta, tilde_nu, delta_x=delta_x_all, output=w_CG[sort_v_all$ix], kernel_type=kernel_type)
...
initiate_grid = function(grid_boundary_info) {
  ...
  neighbor_index_list = vector("list", nx * ny)
  
  # Create direction vectors for the 8 neighbors
  dx = c(-1, 0, 1, -1, 1, -1, 0, 1)
  dy = c(-1, -1, -1, 0, 0, 1, 1, 1)
  
  # Vectorized computation of grid positions
  for(i in 1:(nx*ny)) {
    i_x = (i - 1) %% nx + 1
    i_y = ceiling(i/nx)
    
    # Calculate all potential neighbor positions at once
    neighbor_x = i_x + dx
    neighbor_y = i_y + dy
    
    # Filter valid neighbors using logical indexing
    valid_neighbors = which(
      neighbor_x > 0 & neighbor_x <= nx &
        neighbor_y > 0 & neighbor_y <= ny
    )
    
    # Convert valid positions to indices
    neighbor_index_list[[i]] = c(
      i,  # Include self
      (neighbor_y[valid_neighbors] - 1) * nx + neighbor_x[valid_neighbors]
    )
  }
