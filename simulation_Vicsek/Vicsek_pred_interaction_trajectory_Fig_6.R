...
#seed = 102-121 correspond to n_t=900, T_sim=10, sigma_0=0.1
exp=6
n_repeat=20
i_repeat=1
...
#pdf('Vicsek_pred_CI.pdf',height=3.5,width=4.5)
...
#pdf('Vicsek_pred_insert_CI.pdf',height=2.8,width=3.6)
...
theta_train_vec=est@training_data$training_angles

##last time step from training
p1=as.vector(rbind(sim@px_list[[T_sim+1]],sim@py_list[[T_sim+1]]))
v1=as.vector(rbind(sim@vx_list[[T_sim+1]],sim@vy_list[[T_sim+1]]))
theta1 = sim@theta_list[[T_sim+1]]

##simulated T_test steps
#set.seed(2)
set.seed((exp-1)*n_repeat+i_repeat+1)
m_Vicsek_test=Vicsek(p0=p1,v0=v1,theta0=theta1,n_t=n_t, v_abs=v_abs,T_sim=T_test,h,cut_r,sigma_0)

input_pos_test = m_Vicsek_test$pos
v_test = m_Vicsek_test$v
theta_test=m_Vicsek_test$theta


##predict T_test steps
#radius_rec=cut_r
est_param=c(beta,tau,radius)
#set.seed(2)
set.seed((exp-1)*n_repeat+i_repeat+1)
m_Vicsek_pred=Vicsek_pred_trajec(est_param,v1,p1,v_abs,n_t,T_test,h,sigma_0,weights,theta_train_vec,kernel_type = kernel_type)


pos_pred=m_Vicsek_pred$pos_pred
v_pred=m_Vicsek_pred$v_pred
...
# ggplot(dat_path_truth,aes(s1,s2,group=id))+geom_path(linetype=1,col="black")+
#   geom_segment(data=dat_arrow_truth,aes(x=x,y=y,xend=xend,yend=yend),
#                arrow = arrow(length = unit(0.03, "inches"),type="closed"))+
#   xlim(c(-5,34))+ylim(c(-5,35))+
...
#pdf(paste("pred_traj_",T_test,"_steps.pdf",sep=""),height=3.5,width=4.5)
ggplot(dat_path_truth,aes(s1,s2,group=id))+geom_path(aes(linetype=path,col=path),size=0.65)+
  ...
  geom_segment(data=dat_arrow_pred,aes(x=x,y=y,xend=xend,yend=yend),col="#00618e",
               arrow = arrow(length = unit(0.03, "inches"),type="closed"))+
  xlim(c(-3,34))+ylim(c(-5,33))+
  xlab(expression(s[1]))+ylab(expression(s[2]))+
  ggtitle("Particle trajectories")+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_color_manual(values=c("#fda6b1","#00618e"))+
  theme(panel.background = element_rect(fill = "white", colour = "black"),
        legend.position = c(.5, .85),legend.key = element_rect(fill = "white", colour = "gray50"),
        legend.text=element_text(size=10),legend.title=element_blank(),
        #legend.box.background = element_rect(),legend.box.margin = margin(0,0,0,0),
        axis.text=element_text(size=12),axis.title=element_text(size=15),
        plot.title = element_text(hjust = 0.5,size = 18))
