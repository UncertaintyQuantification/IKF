...
D=2  ##dimension for distance pair
...
beta_est_rec=array(NA,c(3,n_repeat,length(sigma_0_seq)),
                   dimnames = list(c("est","LB","UB"),NULL,c("sigma0.1","sigma0.2")))
...
for(s in 1:2){
  ...
  for(i_repeat in 1:n_repeat){
    ...
    rb = residual_bootstrap_Vicsek(est, sim, B = B,
...
#save.image("Vicsek_residual_boostrap.RData")


####plot
load("Vicsek_residual_boostrap_Jan_25.RData")
#(bootstrap residual) CI for radius with sigma_0=0.1
...
#jpeg("plots/radius_CI_s0.2.jpeg", quality = 100,width = 1250, height = 600, res = 250)
